...
-- 4. Most Expensive Contender
    ...
    SELECT stageName, Highest_Total_Daily_Salary
    FROM(SELECT stageName,CONT_ID, MAX(Total_Daily_Salary_Per_Contender) as Highest_Total_Daily_Salary
        FROM(
    ...
    WHERE Highest_Total_Daily_Salary = (SELECT MAX(Total_Daily_Salary_Per_Contender) FROM(SELECT stageName,CONT_ID, MAX(Total_Daily_Salary_Per_Contender) as Highest_Total_Daily_Salary
        FROM(
        SELECT stageName, CONT_ID, SUM(Participant_Total_Daily_Salary) as Total_Daily_Salary_Per_Contender
        FROM(
            SELECT stageName, PART_ID, CONT_ID,Total_Daily_Salary_Per_Contender_Participant.Total_Daily_Salary as Participant_Total_Daily_Salary
